...
public class Sessions extends AbstractInterestProfile {
	...
	/**
	 *
	 * In dieser Methode wird ein Gesprächskontext empfangen, sobald ein Gespräch
	 * abgeschlossen wird. Anhand der Informationen die innerhalb des empfangenen
	 * Events gespeichert sind, wird ein Protokoll erzeugt. Das ausgehende Format
	 * ist machinell analysiserbar und die Informationen können im späteren Verlauf
	 * auch auf Google Drive gespeichert werden.
	 *
			...
			for (Property<?> property : event.getProperties()) {
				...
				case "topic":
					if (property.getValue() != null) {
						System.out.println(
								"Protokoll hat das folgende Topic empfangen: " + property.getValue().toString());
	...
	/**
	 * Fügt dem protocolEvent für den ProtocolAgent folgende Attribute hinzu: SessionID, SessionStart,
	 * SessionEnd, Duration, User, Topics, ProposedDocs, ClickedDocs, Projects.
	 */
	public void CreateProtocolEvent() {
		try {
			...
			Integer duration = TimeUtils.getDifferenceInSeconds(ProtocolAgent.getSessionEnd(),
					ProtocolAgent.getSessionStart());
			...
			/**
			 * Der ProtocolAgent sendet das ProtocolEvent.
			 */
			try {
